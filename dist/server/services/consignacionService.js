...
//  PROVEEDORAS
// ===================================================
export const getProveedoras = async (companyId) => {
...
export const createProveedora = async (companyId, data) => {
...
//  CONSIGNACIONES
// ===================================================
export const getConsignaciones = async (companyId) => {
...
export const createConsignacion = async (companyId, data) => {
...
// ===================================================
export const getReporteProveedora = async (companyId, proveedoraId) => {
    const proveedora = await prisma.proveedora.findFirst({
        where: { id: proveedoraId, company_id: companyId }
    });
    if (!proveedora)
        throw new Error("Proveedora no encontrada");
    ...
    const detallePorConsignacion = consignaciones.map(c => {
        ...
        return {
            ...
            disponibles: c.cantidad_disponible,
            vendidos: ventasDeEsta.reduce((s, v) => s + v.cantidad_vendida, 0),
            precio_proveedora: c.precio_venta_proveedora,
            precio_tuyo: c.precio_venta_tuyo,
            deuda_a_proveedora: ventasDeEsta.reduce((s, v) => s + v.monto_a_reportar, 0),
            tu_ganancia: ventasDeEsta.reduce((s, v) => s + v.tu_ganancia, 0),
...
//  CREAR LIQUIDACIÓN
// ===================================================
export const crearLiquidacion = async (companyId, proveedoraId, notas) => {
    ...
    const detalles = Array.from(agrupadas.entries()).map(([consignacion_id, data]) => ({
        consignacion_id,
...
export const registrarPago = async (liquidacionId, montoPagado) => {
