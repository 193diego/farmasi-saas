...
router.get("/ventas", authenticateToken, async (req, res) => {
    try {
        ...
        if (desde || hasta) {
            where.fecha_venta = {};
            if (desde)
                where.fecha_venta.gte = new Date(String(desde));
            if (hasta) {
...
router.get("/productos-top", authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.company_id;
        const { desde, hasta } = req.query;
        const where = { venta: { company_id: companyId } };
        if (desde || hasta) {
            where.venta = { ...where.venta, fecha_venta: {} };
            if (desde)
                where.venta.fecha_venta.gte = new Date(String(desde));
            if (hasta) {
                const h = new Date(String(hasta));
                h.setHours(23, 59, 59);
...
// ── REPORTE FINANCIERO ──────────────────────────────────────
router.get("/financiero", authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.company_id;
        const { desde, hasta } = req.query;
        const ventaWhere = { company_id: companyId };
        const gastoWhere = { company_id: companyId };
        if (desde || hasta) {
            const dateFilter = {};
            if (desde)
...
router.get("/cuentas-cobrar", authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.company_id;
