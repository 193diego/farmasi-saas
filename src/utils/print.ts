...
export function printThermal(sale: SaleReceipt) {
  const linea = "─".repeat(32);
  const centro = (txt: string, w = 32) => {
    const pad = Math.max(0, Math.floor((w - txt.length) / 2));
    return " ".repeat(pad) + txt;
  };
  const fila = (izq: string, der: string, w = 32) => {
...
// ── DESCARGA TXT ─────────────────────────────────────────────
export function downloadTxt(sale: SaleReceipt) {
