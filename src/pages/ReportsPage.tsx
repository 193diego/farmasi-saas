...
import {
  TrendingUp, DollarSign, Package, Users, CreditCard, Layers,
  Download, FileText, RefreshCw, Filter, AlertTriangle, Activity, Target,
...
type TabKey = "ventas" | "financiero" | "inventario" | "clientes" | "consignaciones" | "cuentas";
...
const KPI = ({ title, value, sub, icon: Icon, trend, color = "primary" }: any) => {
  const colors: any = {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border hover:shadow-md transition-all" style={{ borderColor: c.br }}>
...
export default function ReportsPage({ sales, products, expenses, customers }: ReportsPageProps) {
  ...
  const [dataClientes, setDataClientes] = useState<any>(null);
  const [dataConsig, setDataConsig] = useState<any>(null);
  const [dataCuentas, setDataCuentas] = useState<any>(null);
  ...
  const exportPDF = async (id: string, nombre: string, subtitulo?: string) => {
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setShowFilters(p => !p)}
            className="flex items-center gap-2 px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-50">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <select value={period} onChange={e => { setPeriod(e.target.value); setDesde(""); setHasta(""); }}
                  className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm outline-none focus:border-[#F45B69]">
      ...
      <div className="flex gap-1 bg-white rounded-2xl p-1.5 border border-gray-100 shadow-sm overflow-x-auto">
        {TABS.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => setActiveTab(key as TabKey)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all flex-shrink-0 ${activeTab === key ? "text-white shadow-sm" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`}
            style={activeTab === key ? { background: GRADIENT } : {}}>
              ...
              {tendencias.length > 0 && (
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={() => exportPDF("chart-ventas", "Ventas")}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => exportToCSV(dataVentas.ventas, "ventas")}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-gray-100 text-gray-600 hover:bg-gray-200">
                      <Download className="w-3.5 h-3.5" />CSV
                    </button>
                    <button onClick={() => exportPDF("tabla-ventas", "Ventas", `Período: ${period}`)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-white hover:opacity-90"
                      style={{ background: GRADIENT }}>
                ...
                <div id="tabla-ventas" className="overflow-x-auto">
                  ...
                  {(!dataVentas.ventas || dataVentas.ventas.length === 0) && (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {tendencias.length > 0 && (
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <button onClick={() => exportPDF("chart-finanzas", "Financiero")}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold text-white" style={{ background: GRADIENT }}>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={() => exportPDF("tabla-financiero", "Estado-Resultados")}
                      className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold text-white" style={{ background: GRADIENT }}>
                      <Download className="w-3 h-3" />PDF
                    </button>
                  </div>
              ...
              {dataFinanciero.gastos_por_categoria?.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={() => exportToCSV(dataFinanciero.detalle_gastos, "gastos")}
                      className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-gray-100 text-gray-600">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <ResponsiveContainer width="100%" height={200}>
                      <PieChart>
                        <Pie data={dataFinanciero.gastos_por_categoria} cx="50%" cy="50%" outerRadius={80} dataKey="monto" nameKey="tipo">
                          {dataFinanciero.gastos_por_categoria.map((_: any, i: number) => (
                    ...
                    <div className="space-y-2">
                      {dataFinanciero.gastos_por_categoria.map((g: any, i: number) => (
                        <div key={g.tipo} className="flex items-center justify-between p-2.5 bg-gray-50 rounded-xl">
            ...
            <>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
                      className="px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-bold outline-none">
                      {cats.map(c => <option key={c} value={c}>{c === "todas" ? "Todas" : c}</option>)}
                    </select>
                    <button onClick={() => exportPDF("tabla-inventario", "Inventario")}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold text-white" style={{ background: GRADIENT }}>
                      <Download className="w-3 h-3" />PDF
                    </button>
                    <button onClick={() => exportToCSV(dataInventario.productos, "inventario")}
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr style={{ background: "#FFF8F6" }}>
                        {["Producto", "Categoría", "Stock", "P. Costo", "P. Venta", "Val. Stock", "Val. Venta", "Margen %", "Estado"].map(h => (
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {(dataInventario.productos || [])
                        .filter((p: any) => categoryFilter === "todas" || p.categoria === categoryFilter)
                        .map((p: any) => (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div id="tabla-clientes" className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr style={{ background: "#FFF8F6" }}>
                        {["#", "Cliente", "Compras", "Total Gastado", "Saldo Pendiente", "Ticket Prom.", "Último Pedido", "Prod. Favorito"].map(h => (
                          <th key={h} className="px-4 py-3 text-xs font-bold uppercase" style={{ color: C.textSub }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {dataClientes.map((c: any, i: number) => (
